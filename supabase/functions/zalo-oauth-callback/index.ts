...
// Public endpoint — this is the redirect_uri Zalo sends the user's browser back
// to after login. Looks up the pending oauth_states row by `state`, exchanges
// the code for a token, fetches the Zalo profile, and stores the connection.
// Shows a simple HTML confirmation page (no app auth context available here).
...
const CALLBACK_URL = `${SUPABASE_URL}/functions/v1/zalo-oauth-callback`

// Fallback for standalone/dev-client builds when the app didn't send a redirectUri
// (e.g. an old client build). Expo Go sessions always send their own exp:// URI.
...
Deno.serve(async (req) => {
  // Reassigned once we've looked up the state row, so the catch block below can
  // still bounce back to the right app build even if a later step throws.
  ...
  try {
    ...
    if (!tokenJson.access_token) {
      console.error('zalo token exchange failed:', JSON.stringify(tokenJson))
      throw new Error(tokenJson.error_description ?? 'Zalo token exchange failed')
    }
    ...
    if (!profileRes.ok || !profile?.id) {
      console.error('zalo profile request failed:', JSON.stringify(profile))
      throw new Error(profile?.message ?? profile?.error_description ?? 'Không lấy được thông tin tài khoản Zalo')
    }
    ...
    return new Response(page('Kết nối Zalo thành công!', `Đã kết nối tài khoản ${profile.name ?? ''}. Bạn có thể quay lại ứng dụng.`, true, appRedirectUrl), {
      headers: { 'Content-Type': 'text/html; charset=utf-8' },
    })
  } catch (error) {
